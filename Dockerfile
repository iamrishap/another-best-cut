...
# Add JS source
COPY ./another_best_cut/static /usr/src/app/another_best_cut/static
...
# Add app source
COPY ./another_best_cut /usr/src/app/another_best_cut
# Add built production JS assets
COPY --from=builder /usr/src/app/another_best_cut/static/dist /usr/src/app/another_best_cut/static/dist

ENV WEB_CONCURRENCY 1
WORKDIR /usr/src/app
EXPOSE 8080
USER nobody
CMD ["gunicorn", "-b", "0.0.0.0:8080", "--access-logfile", "-", "another_best_cut:app"]